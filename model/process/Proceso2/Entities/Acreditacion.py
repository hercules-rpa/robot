from enum import Enum, Flag

"""
TipoAcreditacion es un flag que identifica el tipo de acreditación al que se desea optar
en el proceso de solicitud de acreditaciones (P4 - RPA)
"""
class TipoAcreditacion(Flag):
    NODEFINIDO = 0
    TITULARIDAD = 1
    CATEDRA = 2
    PUP = 3
    PCD = 4
    PAD = 5

class CategoriaAcreditacion(Flag):
    NODEFINIDO =0
    INVESTIGACION=1
    DOCENCIA=2

"""
Clase que define las propiedades de las acreditaciones
    tipo = catedra, titularidad, ...
    nombre = cadena que se utilizará para el desarrollo del informe
"""
class Acreditacion():
    def __init__(self, id, parametros, categoria:CategoriaAcreditacion=CategoriaAcreditacion.NODEFINIDO):
        self.tipo = TipoAcreditacion(int(id))
        self.nombre = parametros['name']
        self.categoria = categoria
